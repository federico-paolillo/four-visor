...
- Every project-controlled local or Compose service listener, proxy target, container, health-check, and published port uses 65100–65199. Remote upstream, ingress, and operator exporter destinations are outside this range rule. Only edge Caddy publishes a host port, explicitly on `127.0.0.1`.
