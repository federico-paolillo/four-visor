...
func TestOTLPLogHandlerFiltersRecordsAndAttributesOnlyFromExport(t *testing.T) {
	...
	if strings.Count(exported.String(), "\n") != 4 || strings.Contains(exported.String(), "routine chatter") {
	...
	for _, forbidden := range []string{
		...
		"error-secret", "policy-secret", "capacity-secret",
	} {
	...
	for _, retained := range []string{
		...
		"lineage.deadline", "resource.queued.count", "resource.rate_capacity.count",
	} {
